...
JWT_SECRET = "aititles_secret_key_2026_change_this"
JWT_EXPIRE_HOURS = 72
...
class UserManager:
    ...
    def login(self, phone, password):
        ...
        try:
            pw_hash = self._hash_password(password)
            c.execute("SELECT id, phone, plan, daily_ai_limit FROM users WHERE phone=? AND password_hash=?", (phone, pw_hash))
            user = c.fetchone()
            if not user:
